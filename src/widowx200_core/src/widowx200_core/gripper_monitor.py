...
class WidowXGripper:
    ...
    def __init__(self, robot_type='wx200', boundaries=False):
        ...
        if self.robot_type == 'wx200':
            self.joints_sub = rospy.Subscriber('/wx200/joint_states',
                                               JointState, self.joint_callback)
            self._gripper_pub = rospy.Publisher('/wx200/single_joint/command',
                                                SingleCommand, queue_size=1)
            self._gripper_desired_sub = rospy.Subscriber(
                '/wx200/single_joint/command', SingleCommand,
                self.gripper_desired_callback)
        elif self.robot_type == 'wx250s':
            self.joints_sub = rospy.Subscriber('/wx250s/joint_states',
                                               JointState, self.joint_callback)
            self._gripper_pub = rospy.Publisher('/wx250s/single_joint/command',
                                                SingleCommand, queue_size=1)
            self._gripper_desired_sub = rospy.Subscriber(
                '/wx250s/single_joint/command', SingleCommand,
                self.gripper_desired_callback)
        else:
    ...
    def monitor_gripper(self):
        ...
        while True:
            ...
            print({'Velocity': self._joint_states.velocity[i], 'Gripper State': \
                self._joint_states.position[i],
                   'Desired Position': self._desired_position})
            if (self._desired_position is not None and
                    self._joint_states.velocity[i] == 0.0 and
                    abs(self._joint_states.position[
                            i] - self._desired_position) > 0.015):
                # When we just give the gripper a commmand, the velocity might be 0
                self._gripper_pub.publish(
                    SingleCommand('gripper',
                                  self._joint_states.position[i] + 0.01)
                )
            time.sleep(1)
...
def main():
    ...
    gripper = WidowXGripper(robot_type='wx250s')
    rospy.spin()

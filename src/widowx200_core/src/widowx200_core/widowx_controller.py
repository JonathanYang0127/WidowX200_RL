...
class WidowXController:
    def __init__(self, robot_type='wx200'):
        ...
        if self.robot_type == 'wx200':
            ...
            self._single_joint_pub = rospy.Publisher('/wx250s/single_joint/command', SingleCommand, queue_size=1)
            self._multiple_joints_pub = rospy.Publisher('/wx250s/joint/commands', JointCommands, queue_size=1)
    ...
    def move_to_target_joints(self, joint_values):
        ...
        for i in range(self.robot_params['NUM_JOINTS'] - 1):
            joint_command[i] = max(joint_values[i], self.robot_params['JOINT_MIN'][i])
            joint_command[i] = min(joint_values[i], self.robot_params['JOINT_MAX'][i])

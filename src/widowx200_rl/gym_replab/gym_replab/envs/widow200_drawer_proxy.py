...
class Widow200DrawerProxyEnv(gym.Env):
    def __init__(self, observation_mode='verbose', reward_type='sparse', grasp_detector='background_subtraction', transpose_image = False, append_image = False):
        #Normalized action space
        self.action_space = spaces.Box(low=np.array([-1, -1, -1, -0.5, -1, -1]),
        ...
        self.num_channels = 3 if not append_image else 6
        self.observation_space = Dict({'state': spaces.Box(low=np.array([-1, -1, -1, -3.0, -3.0, -3.0, -3.0, -3.0, -3.0]),
                                      ...
                                      'image': spaces.Box(low=np.array([0]*self.image_shape[0]*self.image_shape[1]*self.num_channels),
                                            high=np.array([255]*self.image_shape[0]*self.image_shape[1]*self.num_channels), dtype=np.float32)})

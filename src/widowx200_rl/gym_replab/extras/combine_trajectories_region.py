...
training_pool = [gym_replab.utils.DemoPool() for i in range(4)]
...
def add_trajectory(data_file):
    global counter
    param_file = data_file.replace('pool', 'params')
    ...
    for arr_obs, arr_action, arr_nobs, arr_reward, arr_done in zip(data['observations'], data['actions'], \
        ...
        if pool_type < TRAINING_FREQUENCY:
            goal = params['goal']
            if goal[0] < 0.26 and goal[1] <= 0:
                i = 0
            elif goal[0] < 0.26 and goal[1] > 0:
                i = 1
            elif goal[0] > 0.26 and goal[1] <= 0:
                i = 2
            elif goal[0] > 0.26 and goal[1] > 0:
                i = 3
            training_pool[i].add_sample(obs, action, nobs, reward, done)
        else:
            validation_pool[i].add_sample(obs, action, nobs, reward, done)
    if pool_type < TRAINING_FREQUENCY:
        counter += 1
...
def combine_data(data_dirs):
    ...
    if args.batch_size == -1:
        print(save_dir)
        for i in range(4):
            training_pool[i].save(['s'], save_dir,
                f'combined_training_pool_{i}.pkl')

...
def pull_image(image_type_str, color_filter = False):
    image_service = KinectImageService(image_type_str)
    if image_service.image_type in ["rgb", "depth_qhd_dr", "depth_sd_d"]:
            ...
            if color_filter:
                for i in range(image.shape[0]):
                    for j in range(image.shape[1]):
                        if image[i][j][2] < 190 or image[i][j][0] > 150 and \
                            image[i][j][1] > 150:
        ...
        if save_images:
            ...
            plt.imsave("image{}.png".format(0), image.copy())
        rospy.sleep(1)
        return image
    elif image_service.image_type == "sd_pts":
        pc = image_service.get_pc()
        pc_array = image_service.pc_array
        if pc is None:
            print("No points received yet")
        else:
            print(pc_array.dtype, pc_array.shape)
        pcl.save_XYZRGBA(pc, "pc{}.pcd".format(0))
        return pc

if __name__ == '__main__':
    try:
        pull_image("rgb", True) # "rgb" or "sd_pts"
